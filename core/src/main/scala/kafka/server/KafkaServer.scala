 ...
 */
class KafkaServer(val config: KafkaConfig, time: Time = Time.SYSTEM, threadNamePrefix: Option[String] = None, enableForwarding: Boolean = false) extends KafkaBroker with Server {

  //不在任何方法中的代码,叫做构造代码,是在主构造器被调用是执行
  ...
  override def startup(): Unit = {
    try {
      ...
      if (canStartup) {
        ...
        // nio 服务端:SocketServer 是接受客户端socket连接、处理请求并返回处理结果的核心类
