...
class SocketServer(val config: KafkaConfig,
  ...
  private def createControlPlaneAcceptorAndProcessor(endpointOpt: Option[EndPoint]): Unit = {
    //endpointOpt 一台主机可以配置多个kafka实例, node01:9092,node01:9093,node01:9094 --> 伪分布式
    endpointOpt.foreach { endpoint =>
      connectionQuotas.addListener(config, endpoint.listenerName)
      //作用:监听并接受客户端请求,同时建立客户端通道socketChannel,轮询方式交给后端的一个Processor线程处理
...
private[kafka] class Acceptor(val endPoint: EndPoint,
   ...
   */
  def run(): Unit = {
    //向NIOSelector上注册OP_ACCEPT事件,接下来该channel可以接受客户端请求
   ...
   * 获取key对应的socketChannel
   * key表示发生的事件
   */
  private def accept(key: SelectionKey): Option[SocketChannel] = {
    //通过key获取对应的ServerSocketChannel对象
...
private[kafka] class Processor(val id: Int,
  ...
  //每个Processor线程都有自己的SocketChannel队列,保存的是新创建连接的SocketChannel信息,每当Processor线程接收到新连接请求时,都会将对应的SocketChannel放入这个队列中
  private val newConnections = new ArrayBlockingQueue[SocketChannel](connectionQueueSize)
  //临时的response队列,当Processor将Response返回给Request发送方后,还要将Response放入这个临时队列中
    ...
    try {
      while (isRunning) {
        try {
          ...
          //读取每一个socketChannel,然后向selector注册OP_READ事件
          configureNewConnections()
          // register any new responses for writing
          //注册一个Response对象,发送Response,并将其放入队列
          processNewResponses()
          //获取socketChannel上准备就绪的I/O操作
          poll()
          //将接受到的Request请求放入队列
          ...
          case Some(channel) =>
            ...
            if (header.apiKey == ApiKeys.SASL_HANDSHAKE && channel.maybeBeginServerReauthentication(receive,
              ...
              if (channel.serverAuthenticationSessionExpired(nowNanos)) {
                ...
                //移除当前connectionId对应channel的OP_READ事件
