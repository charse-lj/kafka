...
public class KafkaChannel implements AutoCloseable {
     ...
     * <ul>
     *   <li> NOT_MUTED: Channel is not muted. This is the default state. 活动状态</li>
     *   <li> MUTED: Channel is muted. Channel must be in this state to be unmuted. (静音状态)不活动状态</li>
    ...
    public boolean finishConnect() throws IOException {
        ...
        if (connected) {
            if (ready()) {
                //修改SocketChannel对应SocketChannel的状态
                state = ChannelState.READY;
    ...
    /**
     * 是否在活动状态：可读或可写,receive不为空,且分配了数据缓冲区
     *
     * @return .
     */
    ...
    public void setSend(NetworkSend send) {
        ...
        //缓存
        this.send = send;
        //注册一个OP_WRITE事件,一旦绑定该请求,就可以通过该KafkaChannel向服务端发送请求
    ...
    /**
     * 解决粘包问题:长度编码方式 4字节(内容长度)+内容
     * eg. 100nihaowaixx20ha
     *
     * @return
     * @throws IOException
     */
