...
public class NetworkClient implements KafkaClient {
    ...
    public NetworkClient(Selectable selector, Metadata metadata, String clientId, int maxInFlightRequestsPerConnection, long reconnectBackoffMs, long reconnectBackoffMax,
                         int socketSendBuffer, int socketReceiveBuffer, int defaultRequestTimeoutMs, long connectionSetupTimeoutMs, long connectionSetupTimeoutMaxMs, ClientDnsLookup clientDnsLookup,
        ...
        this(null, metadata, selector, clientId, maxInFlightRequestsPerConnection, reconnectBackoffMs, reconnectBackoffMax,
                socketSendBuffer, socketReceiveBuffer, defaultRequestTimeoutMs, connectionSetupTimeoutMs, connectionSetupTimeoutMaxMs,
                clientDnsLookup, time, discoverBrokerVersions, apiVersions, throttleTimeSensor, logContext, new DefaultHostResolver());
    }
    ...
    private void cancelInFlightRequests(String nodeId, long now, Collection<ClientResponse> responses) {
        ...
        for (InFlightRequest request : inFlightRequests) {
            ...
            if (!request.isInternalRequest) {
                if (responses != null) {
                    //对这些请求进行处理,自己封装一个响应,响应消息为null,失去连接的标识为true
    ...
    public boolean isReady(Node node, long now) {
        ...
        // 要发送写数据请求时,不能是正在更新元数据的请求
        // 对应的broker是否建立好连接,并且可以发送数据
    ...
    private boolean canSendRequest(String node, long now) {
        //isReady()生产者缓存了多个连接,一个broker对应一个连接,从缓存中检查是否节点建立好连接
    ...
    private void handleTimedOutRequests(List<ClientResponse> responses, long now) {
        //inFlightRequests 获取所有超时请求的brokerId
        ...
        for (String nodeId : nodeIds) {
            ...
            //创建一个disconnectionl类型的clientResponse对象添加到本地集合
    ...
    private void handleCompletedSends(List<ClientResponse> responses, long now) {
        ...
        for (NetworkSend send : this.selector.completedSends()) {
            //根据主机获取最近一次在inFlightRequests中的请求
            InFlightRequest request = this.inFlightRequests.lastSent(send.destinationId());
            //发送成功,但并不期望服务端响应的请求(不需要)
    ...
    class DefaultMetadataUpdater implements MetadataUpdater {
        ...
        public void handleServerDisconnect(long now, String destinationId, Optional<AuthenticationException> maybeFatalException) {
            ...
            //如果有异常信息,给metadata.fatalException赋值
