...
public class BufferPool {
     ...
     * @param maxTimeToBlockMs The maximum time in milliseconds to block for buffer memory to be available 最大等待时间
     * @return The buffer
    ...
    public ByteBuffer allocate(int size, long maxTimeToBlockMs) throws InterruptedException {
        ...
        try {
            ...
            // poolableSize 是一个批次的大小,默认16kb;需要申请的批次大小:和发往kafka的内容大小有关,如果内容很小,取默认值,如果很大,取内容大小
            ...
            if (this.nonPooledAvailableMemory + freeListSize >= size) {
                ...
                try {
                    ...
                    // 跳出当前循环,表示 已经分配的总内存 >= 需要的内存,重置
                    accumulated = 0;
                } finally {
                    // When this loop was not able to successfully terminate don't loose available memory
                    // 失败时,数据不会丢失,成功时accumulated =0
