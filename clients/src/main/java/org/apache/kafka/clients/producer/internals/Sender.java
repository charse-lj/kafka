...
public class Sender implements Runnable {
        ...
        for (Iterator<Map.Entry<TopicPartition, List<ProducerBatch>>> batchIt = inFlightBatches.entrySet().iterator(); batchIt.hasNext(); ) {
            ...
            if (partitionInFlightBatches != null) {
                ...
                while (iter.hasNext()) {
                    ...
                    //是否过期的标准:系统当前时间与该批次的创建时间差是否超过accumulator.getDeliveryTimeoutMs()(默认配置120s)
    ...
    void runOnce() {
        ...
        //将准备好的请求发送出去,并处理响应
        //如果对应的请求发送成功,对接收到响应进行处理(对于不期望接受到响应的请求,会做默认处理)
        client.poll(pollTimeout, currentTimeMs);
    }

    private long sendProducerData(long now) {
        /**1.获取元数据
         *  {@link MetadataCache#bootstrap(List)}
         */
        Cluster cluster = metadata.fetch();
        // get the list of partitions with data ready to send
        //2.判断哪些partition有消息发送,获取这个partition对应的broker partition对应的主机
        RecordAccumulator.ReadyCheckResult result = this.accumulator.ready(cluster, now);

        // if there are any partitions whose leaders are not known yet, force metadata update
        // 3. 有些partition没有leader信息,更新metadata
        ...
        // 4.遍历所有获取到的主机节点(leader partition 所在的主机节点),基于网络状态检测这些节点是否可用,不可用的移除
        ...
        /** 5. create produce requests
         * 发送的partition有多个,这些partition的leader partition在同一个节点,会将有这些特性的partition进行分组
        ...
        //inFlightBatches中获取超时batch
        List<ProducerBatch> expiredInflightBatches = getExpiredInflightBatches(now);
        //accumulator 的batches 中获取超时batch
    ...
    private void failBatch(ProducerBatch batch,
        ...
        //对于发送异常,且不可重试的请求,也会调用消息的回调方法,并且释放内存
    ...
    private boolean canRetry(ProducerBatch batch, ProduceResponse.PartitionResponse response, long now) {
        return !batch.hasReachedDeliveryTimeout(accumulator.getDeliveryTimeoutMs(), now) &&
                batch.attempts() < this.retries && !batch.isDone() &&
                (transactionManager == null ? response.error.exception() instanceof RetriableException : transactionManager.canRetry(response, batch));
    }
