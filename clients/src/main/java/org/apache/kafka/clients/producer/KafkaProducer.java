...
public class KafkaProducer<K, V> implements Producer<K, V> {
    ...
    /**
     * {@link KafkaProducer#newSender(LogContext, KafkaClient, ProducerMetadata)}
     */
    ...
    KafkaProducer(ProducerConfig config,
        ...
        try {
            ...
            //生产者往服务端发送消息,规定该消息的最大值
            this.maxRequestSize = config.getInt(ProducerConfig.MAX_REQUEST_SIZE_CONFIG);

            //缓冲区的大小,RecordAccumulator大小
            this.totalMemorySize = config.getLong(ProducerConfig.BUFFER_MEMORY_CONFIG);

            //提升发送消息的吞吐量,压缩格式
            this.compressionType = CompressionType.forName(config.getString(ProducerConfig.COMPRESSION_TYPE_CONFIG));

            //最大阻塞时间,当buffer满了或者metadata获取不到等情况下的阻塞时间
            ...
            this.accumulator = new RecordAccumulator(logContext,
                    //RecordAccumulator 中分很多批次,每个批次的大小
                    config.getInt(ProducerConfig.BATCH_SIZE_CONFIG),
                    this.compressionType,
                    //对于批次中的数据,是否等待,默认不等待
                    lingerMs(config), retryBackoffMs,
                    deliveryTimeoutMs, metrics, PRODUCER_METRIC_GROUP_NAME, time, apiVersions, transactionManager,
            ...
            if (metadata != null) {
                ...
                //metadata -->kafka集群的元数据信息,节点、topic、topic分区、分区对应的ISR列表分布在哪些节点
                //获取metadata本质上是需要给kafka集群发送获取kafka集群信息的网络请求
            ...
            //创建一个thread,然后传进去一个Runnable;将业务代码与线程代码隔离
    ...
    Sender newSender(LogContext logContext, KafkaClient kafkaClient, ProducerMetadata metadata) {
        ...
        KafkaClient client = kafkaClient != null ? kafkaClient : new NetworkClient(
                //connections.max.idle.ms 一个网络连接,最多空闲多久,超时关闭 -1 不回收
                new Selector(producerConfig.getLong(ProducerConfig.CONNECTIONS_MAX_IDLE_MS_CONFIG),
                        this.metrics, time, "producer", channelBuilder, logContext),
                metadata,
                clientId,
                //每个网络连接可以忍受producer发送给breaker消息后,无响应的请求数
                maxInflightRequests,
        ...
        return new Sender(logContext,
                ...
                /**
                 * acks
                 * 0: producer只管发送,无返回值,丢失率高
                 * 1: producer发送给 broker后,数据成功写入leader parttion后返回响应,也会丢失数据
                 * -1或者all: 所有副本都返回成功
                 */
                acks,
                //重试次数
    ...
    private static int configureInflightRequests(ProducerConfig config) {
        //max.in.flight.requests.per.connection 发出的请求,其实是有多个网络连接,每个网络连接可以忍受producer发送给breaker消息后,无响应的请求数
        //kafka 有重试机制,所以会造成数据乱序,保证有序,设为1
    ...
    private Future<RecordMetadata> doSend(ProducerRecord<K, V> record, Callback callback) {
        ...
        try {
            ...
            try {
                //1.同步等待拉取topic元数据 --> topic名稱,topic 所属partition,现在时间,
            ...
            //clusterAndWaitTime.waitedOnMetadataMs 代表拉取特定topic元数据用了多长时间
            //剩余时间=最长等待时间-拉取所用时间
            ...
            //7.将消息放入accumulator中(32M内存),由它将消息封装成批次发送
     ...
     * @param topic The topic we want metadata for 当前消息需要发往的topic名称
     * @param partition A specific partition expected to exist in metadata, or null if there's no preference 当前消息需要发往的topic的partition
     * @param nowMs The current time in ms
     * @param maxWaitMs The maximum time in ms for waiting on the metadata 拉取元数据的最大等待时间
    ...
    private ClusterAndWaitTime waitOnMetadata(String topic, Integer partition, long nowMs, long maxWaitMs) throws InterruptedException {
        ...
        //有对应的元数据 --> metadata 内部缓存,不需要远程拉取,拉取时间为0
        ...
        //remainingWaitMs 剩余时间,初次为最大等待时间
        ...
        do {
            ...
            /**
             * 线程唤醒 --> sender线程发送网络请求,拉取元数据
             * 唤醒阻塞于此{@link Selector#select(long)}的sender
             */
            sender.wakeup();
            try {
                //同步等待
            ...
            //获取的分区数partitionsCount(一直取不到元数组) 一直为空,会一直死循环
            partitionsCount = cluster.partitionCountForTopic(topic);
            //获取到的partition总数<=将要发送数据的partition,也会一直循环,大于才能发送数据到对应partition(类似于越界)
